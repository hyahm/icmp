...
func main() {
	...
	p := NewPingOption()
	p.ping3("www.baidu.com")

}

func (p *PingOption) ping3(host string) {

	//查找规范的dns主机名字  eg.www.baidu.com->www.a.shifen.com
	ips, _ := net.LookupIP(host)
	fmt.Printf("%s解析到的ip:%v \n", host, ips)
	starttime := time.Now()
	//此处的链接conn只是为了获得ip := conn.RemoteAddr(),显示出来，因为后面每次连接都会重新获取conn,todo 但是每次重新获取的conn,其连接的ip保证一致么？
	conn, err := net.DialTimeout("ip4:icmp", host, time.Duration(p.Timeout)*time.Millisecond)
	...
	for p.Count > 0 || p.Nerverstop {
		sendN++
		//ICMP报文长度，报头8字节，数据部分32字节
		var msg []byte = make([]byte, p.Size+ECHO_REQUEST_HEAD_LEN)
		//第一个字节表示报文类型，8表示回显请求
		msg[0] = 8 // echo
		//ping的请求和应答，该code都为0
		msg[1] = 0 // code 0
		...
		length := p.Size + ECHO_REQUEST_HEAD_LEN
		//计算检验和。
		...
		conn, err = net.DialTimeout("ip:icmp", host, time.Duration(p.Timeout)*time.Millisecond)
		...
		conn.SetDeadline(starttime.Add(time.Duration(p.Timeout) * time.Millisecond))
